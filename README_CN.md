...
- **手速排行榜**:敲击木鱼 10 次以上(会话内计数)即可上传 —— 昵称(首启自动生成"木鱼玩家#XXXX",随时可改,本地持久化)+ 最快手速(次/秒)进入全球 top10 榜单,自己成绩高亮并显示"我的名次";打破纪录时主页弹窗祝贺(显示超过全球用户比例 x%,来自 GET /percentile),一键"分享记录到全球榜单"(跳上传)或"生成分享图邀请朋友来挑战"(复用分享图组件);网络失败静默降级(榜单显示"加载失败,点击重试");隐私:仅上传昵称与成绩,无任何设备标识,不采集地区
- **手速分享图**:Canvas 自绘 1080×1440 分享卡(壁纸同款色板)—— 品牌 + 最快手速大数字 + 昵称 + 每日最高功德 + 下载二维码(ZXing),自动保存相册(Android 10+ 免权限)并支持系统分享,文案随当前语言
