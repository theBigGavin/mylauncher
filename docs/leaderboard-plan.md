...
### `GET /percentile?rate=23.3`

响应: `{"percentile": 87.5}` — 该成绩超过的全球用户比例(0-100)。用于破纪录弹窗的文案,不产生任何写入。
...
- `gapMs > 0 && gapMs <= 2000`(Gavin 2026-08-19 指令:完全放开下限——娱乐榜,成绩真实性交给 rate/gapMs 互验与样本数;客户端防重入下限 20ms 保证 App 产出的纪录天然 ≥20ms,伪造由互验兜底)
...
1. **设置页彩蛋分组新增"手速排行榜"小节**:
   - 昵称输入框(本地持久化,HomeStore 新键 `leaderboard_nickname`;**首次自动生成**"木鱼玩家#3721"式随机名,零摩擦直接可上传,用户随时可改;不做强制取名弹窗)
...
2. **破纪录弹窗(用户刷新纪录时)**:纪录被打破(`fastestKnockGapMs` 历史纪录更新)时在主页弹窗,文案:"恭喜你刷新了记录,超过了全球 x% 的 MyLauncher 用户!"+ 两个操作按钮:**"分享记录到全球榜单"**(跳上传)+ **"生成分享图邀请朋友来挑战"**(跳分享图)。
   - x% 来自 `GET /percentile`;后端未上线前(A 批)弹窗只显示"恭喜你刷新了记录!"和两个按钮,x% 文案在 B 批接上。
   - 防打扰:只在**打破历史纪录**时弹(会话内刷新不弹),弹窗可关闭,不重复打扰。
3. **手速分享图(纯客户端,不依赖后端,可先行上线)**:
   - 入口:排行榜小节下方"生成分享图"按钮
