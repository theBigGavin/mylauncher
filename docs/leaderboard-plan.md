...
2. **网络**:OkHttp 或 HttpURLConnection(项目无网络依赖,建议直接 HttpURLConnection + kotlinx.coroutines Dispatchers.IO,不加依赖);请求/响应按上面 JSON 契约。超时 10s,失败静默降级(榜单显示"加载失败,点击重试")。
3. **权限/隐私**:仅上报昵称与成绩,无任何设备标识;隐私说明一句放在上传按钮下方("仅上传昵称与手速成绩")。
4. **文案与文案开关**:排行榜小节在彩蛋分组内,未解锁彩蛋不显示;榜单域名抽成常量 `LEADERBOARD_BASE_URL`,便于换环境。

## 里程碑

1. 后端 Worker + 接口自测(curl 跑通三接口 + 校验用例)——后端同事
2. App 端:昵称/上传/榜单 UI + 网络层 + 持久化——客户端
3. 联调(真机上传、榜单刷新、限流/校验边界)
4. 发版:版本按现有制度(新功能=次+1),README 中英文同步

## 风险与明确不做

- 伪造:纯客户端可无限伪造,娱乐定位,不做真防伪;后续若要严肃可加"连击间隔序列"服务端分布校验(仍有伪造空间)。
- 不做账号体系、不做社交(评论/关注)、不做地区榜。
